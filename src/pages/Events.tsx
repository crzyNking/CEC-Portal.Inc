  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] overflow-x-hidden">
      ...
      <div className="bg-[#0a1f44] pt-[120px] pb-20 text-center px-4">
        <h1 className="text-[2rem] sm:text-[2.5rem] md:text-[2.8rem] font-bold text-white mb-4 tracking-wide">Events</h1>
        <p className="text-white/80 max-w-2xl mx-auto text-[12px] sm:text-[13px] leading-relaxed">
          Stay updated with upcoming events, activities, and celebrations at Cebu Eastern College
        </p>
      </div>
      ...
      <section className="max-w-[1100px] mx-auto px-4 sm:px-6 -mt-8 relative z-10 pb-16 sm:pb-20">
        {loading ? (
          ...
          <div className="text-center py-20">
            <p className="text-[#666] text-sm">No events available yet.</p>
          </div>
        ) : (
            ...
            {events.map((item) => (
              <div key={item.id} className="bg-white rounded-xl overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.08)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)] transition-shadow">
                <div className="relative h-[200px] sm:h-[240px]">
                  <img src={item.image_url || 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?auto=format&fit=crop&w=600&q=80'} alt={item.title} className="w-full h-full object-cover" />
                  <div className="absolute top-4 left-4 flex gap-2">
                    <span className="bg-[#0a3182] text-white text-[10px] font-bold px-3 py-1 rounded tracking-wide uppercase">
                      {isPast(item.event_date) ? 'Past Event' : 'Upcoming'}
                    </span>
                    {item.is_featured && (
                ...
                <div className="p-6 sm:p-8">
                  ...
                  <p className="text-[#666] text-[12px] sm:text-[13px] leading-relaxed mb-4">{item.description}</p>
