...
export default function AdminServices() {
  ...
  if (editing) {
    return (
      <div>
        <div className="flex items-center justify-between mb-6"><h1 className="text-2xl font-bold text-[#0B1F3A]">{editing.id ? 'Edit' : 'New'} Service</h1><button onClick={() => setEditing(null)} className="text-sm text-gray-500 hover:text-gray-700">Cancel</button></div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">        <h1 className="text-2xl font-bold text-[#0B1F3A]">Services</h1><button onClick={() => setEditing({ ...empty })} className="px-4 py-2 bg-[#1E4E8C] text-white rounded-lg text-sm font-medium hover:bg-[#0B1F3A]">+ New</button></div>
      ...
      : items.length === 0 ? <div className="bg-white/90 backdrop-blur-sm rounded-xl p-12 text-center text-gray-400 border border-[rgba(11,31,58,0.08)] shadow-[0_4px_20px_rgba(11,31,58,0.06)]">No services yet.</div>
      : (
        <div className="bg-white/90 backdrop-blur-sm rounded-xl border border-[rgba(11,31,58,0.08)] shadow-[0_4px_20px_rgba(11,31,58,0.06)] overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-[#F8FAFC] border-b border-[rgba(11,31,58,0.08)]"><tr><th className="text-left px-4 py-3 font-medium text-gray-600">Title</th><th className="text-left px-4 py-3 font-medium text-gray-600">Status</th><th className="text-right px-4 py-3 font-medium text-gray-600">Actions</th></tr></thead>
            <tbody>
              {items.map((s) => (
                <tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 text-right space-x-2"><button onClick={() => setEditing(s)} className="text-[#1E4E8C] hover:text-[#0B1F3A] hover:underline text-xs font-medium">Edit</button><button onClick={() => remove(s.id)} className="text-red-500 hover:underline text-xs font-medium">Delete</button></td>
                </tr>
              ))}
            </tbody>
          </table>
