...
export default function AdminPrograms() {
  ...
  async function save() {
    ...
    try {
      ...
      if (editing.id) {
        const { error } = await supabase.from('programs').update({ ...editing, slug }).eq('id', editing.id)
        if (error) throw error
        await logAdminActivity('updated', 'program', editing.id, { title: editing.title })
      } else {
        const { data, error } = await supabase.from('programs').insert([{ ...editing, slug }]).select().single()
        if (error) throw error
  ...
  if (editing) {
    return (
      <div>
        <div className="flex items-center justify-between mb-6"><h1 className="text-2xl font-bold text-[#0B1F3A]">{editing.id ? 'Edit' : 'New'} Program</h1><button onClick={() => setEditing(null)} className="text-sm text-gray-500 hover:text-gray-700">Cancel</button></div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">        <h1 className="text-2xl font-bold text-[#0B1F3A]">Programs</h1><button onClick={() => setEditing({ ...empty })} className="px-4 py-2 bg-[#1E4E8C] text-white rounded-lg text-sm font-medium hover:bg-[#0B1F3A]">+ New</button></div>
      ...
      : items.length === 0 ? <div className="bg-white/90 backdrop-blur-sm rounded-xl p-12 text-center text-gray-400 border border-[rgba(11,31,58,0.08)] shadow-[0_4px_20px_rgba(11,31,58,0.06)]">No programs yet.</div>
      : (
            ...
            <tbody>
              {items.map((p) => (
                <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 text-right space-x-2"><button onClick={() => setEditing(p)} className="text-[#1E4E8C] hover:text-[#0B1F3A] hover:underline text-xs font-medium">Edit</button><button onClick={() => remove(p.id)} className="text-red-500 hover:underline text-xs font-medium">Delete</button></td>
                </tr>
              ))}
            </tbody>
          </table>
